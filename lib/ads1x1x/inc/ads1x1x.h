 ...
 * saglayacagi callback fonksiyonlari uzerinden yapilir (bkz. ads1x1x_i2c_read_fn /
 * ads1x1x_i2c_write_fn), boylece STM32 HAL/LL veya baska bir platforma
 * dogrudan bagli degildir.
 */
...
/* ------------------------------------------------------------------------ */
#define ADS1X1X_CFG_OS_BIT           15u  /* 1 = single-shot baslat / 1 = conversion bitti (okunurken) */
 ...
 * (OS=1, MUX=AIN0-AIN1, PGA=+-2.048V, MODE=single-shot, DR=orta hiz, COMP disabled) */
...
typedef enum {
    ...
    ADS1X1X_DR_IDX2 = 2,
    ADS1X1X_DR_IDX3 = 3,
    ADS1X1X_DR_IDX4 = 4, /* POR default (12-bit ailede 1600SPS, 16-bit ailede 128SPS) */
    ADS1X1X_DR_IDX5 = 5,
    ADS1X1X_DR_IDX6 = 6,
    ADS1X1X_DR_IDX7 = 7
} ads1x1x_datarate_t;
...
/* ------------------------------------------------------------------------ */
typedef ads1x1x_status_t (*ads1x1x_i2c_read_fn)(uint8_t dev_addr, uint8_t reg_addr,
...
/** Config register'ini anlamli alanlardan (mux/pga/mode/data_rate) uretip yazar.
 *  ADS1014 icin mux != ADS1X1X_MUX_AIN0_AIN1 istenirse ADS1X1X_ERROR_INVALID_PARAM doner. */
...
/** Config register'ina ham 16-bit deger yazar (dusuk seviye erisim / test icin). */
...
/** Ham degeri, verilen PGA araligina ve variant'in cozunurlugune gore milivolta cevirir. */
ads1x1x_status_t ads1x1x_raw_to_millivolts(const ads1x1x_t *dev, int16_t raw_value,
                                            ads1x1x_pga_t pga, float *millivolts);

/** Verilen data_rate index'inin, aktif variant icin karsilik geldigi gercek SPS degeri. */
