...
/* Right-shift amount that only applies to the 12-bit family (ADS1014/1015):
 * the 12-bit result in the conversion register is left-aligned within the
 * 16-bit field. */
#define ADS1X1X_12BIT_RIGHT_SHIFT 4u

/* 12-bit family (ADS1014/1015) DR index -> SPS */
static const uint16_t ADS101X_DR_TABLE[8] = {128, 250, 490, 920, 1600, 2400, 3300, 3300};

/* 16-bit family (ADS1115) DR index -> SPS */
static const uint16_t ADS111X_DR_TABLE[8] = {8, 16, 32, 64, 128, 250, 475, 860};

/* PGA index -> full-scale range (mV), used for the voltage conversion in
 * this driver */
static const float ADS1X1X_PGA_FSR_MV[6] = {6144.0f, 4096.0f, 2048.0f, 1024.0f, 512.0f, 256.0f};

static bool ads1x1x_variant_has_mux(ads1x1x_variant_t variant)
{
    /* ADS1014 has no physical MUX; it only has a fixed AIN0-AIN1 differential
     * input. */
    return (variant == ADS1X1X_VARIANT_ADS1015) || (variant == ADS1X1X_VARIANT_ADS1115);
}
                              ...
                              ads1x1x_i2c_read_fn read_cb, ads1x1x_i2c_write_fn write_cb)
{
    ...
    if (variant != ADS1X1X_VARIANT_ADS1014 && variant != ADS1X1X_VARIANT_ADS1015 &&
...
ads1x1x_status_t ads1x1x_write_config_raw(ads1x1x_t *dev, uint16_t config_val)
{
    ...
    ads1x1x_status_t status = dev->write(dev->dev_addr, ADS1X1X_REG_CONFIG, data, 2);
...
ads1x1x_status_t ads1x1x_read_config_raw(ads1x1x_t *dev, uint16_t *config_val)
{
    ...
    uint8_t buffer[2];
    ads1x1x_status_t status = dev->read(dev->dev_addr, ADS1X1X_REG_CONFIG, buffer, 2);
    if (status == ADS1X1X_OK)
    {
                                   ...
                                   ads1x1x_mode_t mode, ads1x1x_datarate_t data_rate)
{
    ...
    /* ADS1014 has no MUX: requesting a channel other than the default is an
     * error. */
    ...
    config_val |= (uint16_t)(1u << ADS1X1X_CFG_OS_BIT); /* start a new conversion */
    ...
    config_val |= (uint16_t)(ADS1X1X_CFG_COMP_QUE_DISABLE << ADS1X1X_CFG_COMP_QUE_SHIFT);
...
ads1x1x_status_t ads1x1x_start_conversion(ads1x1x_t *dev)
{
    ...
    uint16_t config_val = dev->config_shadow | (uint16_t)(1u << ADS1X1X_CFG_OS_BIT);
    return ads1x1x_write_config_raw(dev, config_val);
}
...
ads1x1x_status_t ads1x1x_read_raw(ads1x1x_t *dev, int16_t *raw_value)
{
    ...
    uint8_t buffer[2];
    ads1x1x_status_t status = dev->read(dev->dev_addr, ADS1X1X_REG_CONVERSION, buffer, 2);
    if (status != ADS1X1X_OK)
    {
        return status;
    }
    ...
    {
        /* ADS1014/1015: the 12-bit result is left-aligned; shift right to
         * normalize it. */
                                           ...
                                           ads1x1x_pga_t pga, float *millivolts)
{
    ...
    /* Full-scale code: 2^15 for the 16-bit family, 2^11 for the (normalized)
     * 12-bit family. */
    float full_scale_code = ads1x1x_variant_is_16bit(dev->variant) ? 32768.0f : 2048.0f;
